...
public class TC10_CheckUpdateWhenApprStrtEndDateOnHold extends LoginPage
{
	...
	@Test
	public static void UpdateApprDateOnHold() throws InterruptedException, IOException
	{    
		...
		WebElement date=rp.getApprStartDate();
		date.click();
		if(date.isDisplayed())
		{
	    date.click();
		...
		rp.getApprEndMon(mon);
		rp.getApprEndYear(year);
		driver.findElement(By.xpath("//span[contains(text(),'20')]")).click();
		Thread.sleep(5000);
		System.out.println("On Hold Review Status Able To Update the Approval Start Date And End Date");
		}
		else
		{
			System.out.println("On Hold Review Status Unable To Update the Approval Start Date And End date");
		}
		logOut();
